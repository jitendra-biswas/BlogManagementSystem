...
const BlogEditor = () => {
  ...
  return (
    <div className="flex">
      ...
      <form onSubmit={formHandeler} className="right w-[80%] min-h-screen pt-25 p-10">
        {/* Image upload */}
        <div className="thumbnail w-25 h-17 bg-zinc-100 outline-2 outline-gray-500/15 rounded overflow-hidden">
          <label htmlFor="file" className="w-full h-full flex flex-col justify-center items-center cursor-pointer">
            ...
            {Image && <img src={URL.createObjectURL(Image)} alt="" className="w-full h-full visible" />}
          </label>
        </div>
            ...
            {Buttons.map((item) => (
              <p
                ...
                className={`w-fit px-5 py-1 rounded-full cursor-pointer active:scale-95 ${
                  active === item ? "bg-black text-white" : "bg-[#F0F0F0] text-black"
                }`}
              >
                {item}
              </p>

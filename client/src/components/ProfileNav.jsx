...
const ProfileNav = (props) => {
    ...
    function signOut(){
        ...
        axios.get("http://localhost:3000/api/auth/signOut",{withCredentials:true}).then(res=>{
    ...
    useEffect(() => {
       axios.get("http://localhost:3000/api/auth/getUsers",{withCredentials:true}).then(res=>{
        setUserDetail(res.data.user.username)
       })
    }, [])
  ...
  return (
   <>
    <div className={`profileNav fixed z-20 right-16 rounded top-14 pb-10 border-2 border-zinc-300/10 w-60 bg-[#fbfbfb] flex flex-col ${props.visible} ${profileVisible}`}>
        ...
        <p className='w-full px-6 text-sm text-gray-400'>{"@"+userDetail}</p>
    </div>
   </>
  )
}

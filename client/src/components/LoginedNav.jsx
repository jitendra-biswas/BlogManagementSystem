...
const LoginedNav = () => {
  const [sideNavVisiblility, setsideNavVisiblility] = useState("hidden");
  const [profileCardVisible, setprofileCardVisible] = useState("hidden");
  const navigate = useNavigate();

  function showSideNav() {
  setsideNavVisiblility(prev => prev === "hidden" ? "visible" : "hidden");
}
    ...
    {/* navigation bar */}
      <nav className="fixed left-0 top-0 z-30 bg-white w-full flex h-18 items-center justify-between py-3 px-14 border-b-2 border-b-gray-600/10 max-md:px-8">
        ...
        <div className="right flex">
          <button onClick={()=>navigate("/dashboard/blogEditor")} className="text-gray-500 px-5 py-1.5 pb-2 rounded-full text-md font-medium cursor-pointer max-md:hidden ">
